...
class ban_give_out:
    ...
    async def ban_give(self, apis, create_mongo, peer_id, cause, chat_id, user_id, from_id, vrem, ply):
        ...
        res_ban = await create_mongo.ban_check(user_id, peer_id, cause, int(vrem) + result[0], result[0], from_id)
        ...
        res = []
        msg_n = ""
        res = await record_achievements(create_mongo, user_id).run(kol_ban=res_ban)
        if res[1]:
            msg_n = "\n\n👻 Полученные ачивки:\n" + "\n".join(res[1])
        ...
        if res[0] == 1:
            ...
            await create_mongo.ban_check(user_id, peer_id, cause, 604800 + result[0], result[0], from_id)
