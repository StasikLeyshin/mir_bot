    ...
    async def run(self):
        try:
            ...
            if user_id:
                ...
                ban_give = BanGive(self.mongo_manager, self.settings_info, self.from_id, self.date)
                ...
                # user_id = await self.getting_user_id()
                # if user_id:
                #     await achievements(self.client, int(user_id), self.v).add_ban(self.apis, self.peer_id,
                #                                                                   self.from_id, vrem, cause)
