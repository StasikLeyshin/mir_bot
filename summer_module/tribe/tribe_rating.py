...
class TribeRating(WorkUser):
    ...
    async def _get_tribe_rating(self, peer_id: int = 0):
        ...
        user_info_cmd = await self.lvl_cmd_add_list(user_info.class_dict, "tribe_rating")
        msg = ""
        error = False
        user_ids = []
        user_ids_new = []
        ...
        if "tribe_rating" in user_info_cmd or (await self.is_admin(self.user_id, str(peer_id)))['flag']:
            ...
            for i in self.settings_info["tribe"]:
                count = 0
                users_list = await self.manager_db.tribe_users_get(i["settings"]["cut"], self.users_documents)
                for j in users_list:
                    count += j['tribe_points']
                users_list = sorted(users_list, key=self.get_name, reverse=True)
                #tribe_dict.append({"tribe_points": count})
                ...
                for j in users_list:
                    if k == 5:
                        break
                    people_list.append(str(k + 1) + ". -- " + f"[id{j['user_id']}|" + "{users[" + str(index) + "]} -- " + "]" + str(j['tribe_points']))
                    user_ids.append(j['user_id'])
                    k += 1
                    index += 1
                tribe_list.append({"tribe_points": count, "user_ids": user_ids, "name": i["settings"]["name"],
                                   "top": "👑 Топ 5 участников клана:\n" + "\n".join(people_list)})

            #print(tribe_list)
            users_list = sorted(tribe_list, key=self.get_name, reverse=True)
            #print(users_list)
            # for i in zip(users_list):
            #     print(i)
            # for i in users_list:
            #     user_ids_new += i["user_ids"]
            msg = f"👥 Рейтинг кланов:\n\n" +\
                  "\n\n \n".join([f"{tribe_rating[index]}\n"
                               f"Клан: {value['name']}\n"
                               f"🌐 Очки клана: {value['tribe_points']}\n\n"
                               f"{value['top']}" for index, value in enumerate(users_list)])

            if user_info.cmd["tribe_rating"].get("count"):
                user_info.cmd["tribe_rating"]["count"] += 1
            else:
                user_info.cmd["tribe_rating"]["count"] = 1
            user_info.update = True

            await self.add_log_user(user_info, "tribe_rating", await self.get_log_users(self.user_id, peer_id))
...
if __name__ == "__main__":
    ...
    ban = TribeRating(mongo_manager, settings_info, 55, 100)
